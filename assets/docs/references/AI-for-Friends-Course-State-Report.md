...
"AI for Friends" is a 5-module, 21-lesson crash course teaching non-technical professionals (HR, Legal, Marketing, Industrial Engineering, International Relations) to use Claude effectively — from foundational concepts through prompting technique, hands-on Claude Code file work, building a reusable claude.ai Project, to automating recurring multi-step workflows. It's part of effectiveatlife.com, built with the same no-build, single-self-contained-HTML-file-per-page approach as the rest of the site (see root `CLAUDE.md`).

**Current scope:** 27 HTML files total — 1 course index, 5 module hubs, 21 lesson pages. Modules 4 and 5 were ingested in a second batch after this report's initial version (which covered only Modules 1–3); the course map, HTML structure notes, and known-inconsistencies sections below have all been updated to reflect the full 5-module state. There is no Module 6 planned or referenced anywhere in the course as of this writing.

**Source of truth split:**
- Raw content lives as markdown in `assets/docs/ai-for-friends/*.md` (26 files — synthesized from an external private "second-brain" vault, per root CLAUDE.md's content-sourcing rules).
...
| 4 — Building Your AI Personal Assistant | hub | `4-0_Claude_Code_Projects.md` | `module-4/index.html` | Module overview; claude.ai Projects concept; Key Concepts glossary |
| 4 | 4.1 | `4-1_Claude_Code_Project_Basics.md` *(sic, see filename-mismatch note below)* | `module-4/lesson-1.html` | The three parts of a Project (name, custom instructions, Project knowledge); why a Project beats a blank chat |
| 4 | 4.2 | `4-2_First_Project.md` | `module-4/lesson-2.html` | Creating a Project; the four sections of good custom instructions; copy-ready templates for all five roles |
| 4 | 4.3 | `4-3_Project_Material.md` | `module-4/lesson-3.html` | Uploading Project knowledge; what a Project can and cannot reach by default |
| 4 | 4.4 | `4-4_Project_Instructions.md` | `module-4/lesson-4.html` | Where a Project's "memory" lives; keeping standing instructions current |
| 4 | 4.5 | `4-5_Using_Projects_to_Design_More.md` | `module-4/lesson-5.html` | Using a working Project to design the next one via a meta-prompt |
| 4 | 4.6 | `4-6_Examples_by_Role.md` | `module-4/lesson-6.html` | Five complete, finished Projects (Legal, Marketing, Industrial Engineering, International Relations, HR) |
| 5 — Automating Workflows with Claude Code | hub | `5-0_Claude_Code_Workflows.md` | `module-5/index.html` | Module overview; skill vs. plugin concept; "Which Lesson Is For You" role-relevance table |
| 5 | 5.1 | `5-1_Claude_Code_Daily_Habits.md` | `module-5/lesson-1.html` | Session hygiene, fast diff review, usage awareness, the four rules for anything unattended |
| 5 | 5.2 | `5-2_Installing_Antrhopic_Skills.md` *(sic, source filename typo)* | `module-5/lesson-2.html` | Installing Anthropic's `document-skills` plugin (docx/pdf/pptx/xlsx) |
| 5 | 5.3 | `5-3_Automating_Agreements_RI_Example.md` **(0-indexed Step 0–6, see §3)** | `module-5/lesson-3.html` | Full worked example: dictate-to-setup a 3-skill plugin (check webpage → draft follow-up → build Word doc) |
| 5 | 5.4 | `5-4_What_is_an_MCP_Server.md` | `module-5/lesson-4.html` | What an MCP connection is; adding one via claude.ai Connectors and `claude mcp add` |
| 5 | 5.5 | `5-5_Daily_Email_Briefing.md` | `module-5/lesson-5.html` | Connecting email/calendar via MCP; one single-skill daily briefing |
| 5 | 5.6 | `5-6_Market_Awareness.md` | `module-5/lesson-6.html` | Two-skill plugin: monitor sources, report only what's new, email a digest |
| 5 | 5.7 | `5-7_Content-Generation.md` | `module-5/lesson-7.html` | Three-skill plugin: gather articles → compile digest → draft on-brand content (course's final lesson) |
| — | — | *(no single 1:1 source)* | `ai-for-friends/index.html` | Course hub — syllabus overview, links to all 5 module hubs |
...
**Filename-mismatch note (repeats the `1-4_Chossing`/`2-2_Intalling` pattern from Modules 1–2):** Module 4's own hub file (`4-0`) cross-references its lessons using idealized filenames that don't match what was actually delivered — e.g. `4-0` cites `4-1_What_Is_A_Claude_Project.md` and `4-2_Creating_Your_First_Project.md`, but the real files on disk are `4-1_Claude_Code_Project_Basics.md` and `4-2_First_Project.md`. Same issue for several other 4.x/5.x cross-links. This doesn't affect the HTML output (converted by hand against the actual files, not by following the markdown's internal links), but it's a trap for anyone scripting a conversion pipeline off these cross-references — always resolve lesson number → actual filename from the directory listing, not from another file's citation of it.
...
**Module 5's hub (`5-0`) has one section not present in any other hub**: "Which Lesson Is For You" — a table mapping each of the five target roles to which lesson was written specifically for them (5.3, 5.5, or 5.6) versus which lessons' *shape* still transfers even without a role-specific worked example. This exists because Module 5, unlike Modules 1–4, has several lessons built around one specific role's scenario (International Relations for 5.3, Marketing for 5.6) rather than covering all roles evenly in every lesson — the table exists to point non-IR/non-Marketing readers at the right lesson to read closely versus skim.
...
```
`scroll-margin-top: 6rem` on the `h2` is what keeps `#step-N` anchor jumps from hiding under the fixed header. Sequential integers starting at 1, except `module-3/lesson-1.html` which is 0-indexed (`step-0`–`step-5`, mirroring its 6-section source) and uses a larger, purple-tinted variant of this CSS (`font-size: 1.7rem`, `background: rgba(123,44,191,0.12)`, `color: #B18CE0`) to match its "big lesson" theme. **`module-5/lesson-3.html` is also 0-indexed (`step-0`–`step-6`) but deliberately keeps the standard cyan step-num CSS** — confirming the oversized/purple treatment is specific to `3-1`'s single-giant-lesson framing, not a rule that "0-indexed lessons get bigger step-nums."
...
Module hubs also carry a "Lessons in this module" card list before the glossary, and an "Up next" gradient card after Support Resources pointing at the next module — absent only on the *current* last module's hub (Module 5's, as of this writing).

**Module identity colors, updated for all 5 modules:** `01` AI Foundations = `accent-cyan`; `02` Setting Up Your Computer = `os-tertiary` (muted, signals optional); `03` Claude Code = `accent-purple`; `04` Building Your AI Personal Assistant = `accent-teal`; `05` Automating Workflows with Claude Code = `os-primary`/white. Module 5 deliberately does **not** use `accent-amber` — the Design System Guide reserves amber for `callout-warning` only, so it wasn't repurposed as a 5th module numeral color; white was used instead as a distinct, unclaimed identity color. These colors apply only to the module numeral/badge (in the course-menu dropdown, course-index cards, and each module's own hero numeral) — lesson step-num badges stay the standard cyan variant regardless of module, except `module-3/lesson-1.html`'s purple/oversized exception noted above.

**The "terminal module" markers migrate forward each time a module is appended** — this is now an established, expected part of the ingestion process, not a one-off fix: when Module 3 was the last module, its hub had no "Up next" card and its one lesson's closing card read "Course complete" with a "Back to Course Home" nav button. When Module 4 was added, Module 3's hub gained an "Up next" card pointing at Module 4, and its lesson's closing card/nav changed to "Module 3 complete" / "Next: Module 4". The same migration now needs to happen to Module 5 (currently "Course complete", no forward pointer) whenever a Module 6 is eventually added.
...
**Module identity colors** (numeral/badge only, not step-nums): `01`=`accent-cyan`, `02`=`os-tertiary`, `03`=`accent-purple`, `04`=`accent-teal`, `05`=`os-primary`/white.
...
- **Docs/support link domain drift** — three variants for the same two links across pages:
  - `code.claude.com/docs` + `support.claude.com` — most pages (Module 1 hub/lessons 1–3, `module-2/lesson-3.html`, Module 3 hub/lesson, and **all of Module 4 and Module 5**)
